...
class ProfileController extends MemberbaseController2 {
    ...
    public function edit_post() {
    	if(IS_POST){
    		...
    		if ($this->users_model->field('id,user_nicename,sex,birthday')->create()!==false) {
				if ($this->users_model->save()!==false) {
					$this->user=$this->users_model->find($this->userid);
					sp_update_current_user($this->user);
					$this->success("保存成功！",U('zhubo/profile/center'));
				} else {
					$this->error("保存失败！");
				}
			} else {
    ...
    public function recharge_post()
    {
        if(IS_POST)
        {
            ...
            if ($r!==false) {
                $this->success("充值成功！",U('zhubo/profile/center'));
            } else {
                $this->error("充值失败！");
            }
        }
    		...
    		if(sp_compare_password($old_password, $admin['password'])){
    			if($password==I('post.repassword')){
    				if(sp_compare_password($password, $admin['password'])){
    					...
    					$r=$this->users_model->save($data);
    					if ($r!==false) {
    						$this->success("修改成功！",U('zhubo/profile/center'));
    					} else {
    						$this->error("修改失败！");
    					}
    				}
