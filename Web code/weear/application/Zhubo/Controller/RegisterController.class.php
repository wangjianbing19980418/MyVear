...
class RegisterController extends HomebaseController2
{
...
// 	    }
        ...
        if ($result || $uc_checkemail < 0 || $uc_checkusername < 0) {
            ...
            if ($uc_register) {
                ...
                new \User\Controller\ProfileController();
                $result = M('Users');
                $uid = sp_get_current_userid();
                $data['id'] = $uid;
                $data['user_role'] = 'zhubo';
                $result->save($data);
                $rst = $users_model->add($data1);
                if ($rst) {
                    //注册成功页面跳转
                    $data1['id'] = $rst;
                    session('user', $data1);
                    ...
                    if ($need_email_active) {
                        ...
                        session('user', null);
                        $this->success("注册成功，激活后才能使用！", U("Zhubo/login/index"));
                    } else {
                        $this->success("注册成功！", U("Zhubo/profile/center"));
                    }

                } else {
                    $this->error("注册失败！", U("Zhubo/register/index"));
                }
    ...
    public function active()
    {
        ...
        if ($find_user) {
            ...
            if ($result) {
                ...
                $this->success("用户激活成功，正在登录中...", U("zhubo/profile/center"));
            } else {
                $this->error("用户激活失败!", U("user/profile/center"));
            }
        } else {
            $this->error("用户激活失败，激活码无效！", U("user/profile/center"));
        }
