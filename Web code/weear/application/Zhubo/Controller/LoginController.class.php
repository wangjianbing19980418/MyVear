...
class LoginController extends HomebaseController2 {
	...
	protected  function _send_to_resetpass($user){
		...
		//生成激活链接
		$url = U('zhubo/login/password_reset',array("hash"=>$activekey), "", true);
		//邮件内容
		$template =<<<hello
		#username#，你好！<br>
		请点击或复制下面链接进行密码重置：<br>
...
hello;
		...
		if(IS_POST){
			if(!sp_check_verify_code()){
				...
				if($users_model->validate($rules)->create()===false){
					...
					$hash=I("post.hash");
					$result=$users_model->where(array("user_activation_key"=>$hash))->save(array("password"=>$password,"user_activation_key"=>""));
					if($result){
						$this->success("密码重置成功，请登录！",U("zhubo/login/index"));
					}else {
    ...
    public function dologin(){
    	...
    	$rules = array(
    			//array(验证字段,验证规则,错误提示,验证条件,附加规则,验证时间)
    			array('username', 'require', '主播名不能为空！', 1 ),
    			array('password','require','密码不能为空！',1),
    ...
    private function _do_email_login(){
                ...
                switch ($uc_uid){
                    case "-1"://用户不存在，或者被删除
                        if($result){//本应用已经有这个用户
                            if(sp_compare_password($password, $result['password'])){//本应用已经有这个用户,且密码正确，同步用户
        ...
        //exit();
        if(!empty($result)){
            if(sp_compare_password($password, $result['password'])|| $ucenter_login_ok){
                session('user',$result);
                //写入此次登录信息
                $data = array(
                    'last_login_time' => date("Y-m-d H:i:s"),
                ...
                $this->success("登录验证成功！", U('zhubo/profile/center'));
            }else{
                $this->error("密码错误！");
            }
        }else{
