...
class HomebaseController2 extends AppframeController {
    ...
    protected function check_login(){
        $session_user=session('user');
        if(empty($session_user)){
            $this->error('您还没有登录！',leuu('zhubo/login/index',array('redirect'=>base64_encode($_SERVER['HTTP_REFERER']))));
        }
    ...
    protected function  check_user(){
        $user_status=M('Zhubo')->where(array("id"=>sp_get_current_userid()))->getField("user_status");
        if($user_status==2){
            $this->error('您还没有激活账号，请激活后再使用！',U("Zhubo/login/active"));
        }
    ...
    protected  function _send_to_active(){
        ...
        //生成激活链接
        $url = U('zhubo/register/active',array("hash"=>$activekey), "", true);
        //邮件内容
