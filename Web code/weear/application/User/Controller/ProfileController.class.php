...
class ProfileController extends MemberbaseController {
    	...
    	if(IS_POST){
    		...
    		if ($this->users_model->field('id,user_nicename,sex,birthday,user_url,signature')->create()!==false) {
				if ($this->users_model->save()!==false) {
					...
					$this->success("保存成功！",U('user/profile/center'));
				} else {
					$this->error("保存失败！");
				}
			} else {
    ...
    public function zhuboregister() {
        $this->assign($this->user);
        $this->display();
    }

    public function zhuboregiser_post(){
	    if(IS_POST){
	        $this->doregister();
        }
    }
    ...
    public function recharge_post()
    {
        if(IS_POST)
        {
            ...
            else if(!preg_match('/^[0-9]+$/',$money)){
                $this->error("充值数量只能是整数！");
            }
            $uid=sp_get_current_userid();
            $admin=$this->users_model->where(array('id'=>$uid))->find();
            $data['money']=$money+$admin['money'];
            $data['coin']=$admin['coin']+$money;
            $data['id']=$uid;
            if($data['coin']>0){
                if($data['coin']<200)
                    $data['degree']='铜牌会员';
                else if($data['coin']<500)
                    $data['degree']='铁牌会员';
                else if($data['coin']<1000)
                    $data['degree']='银牌会员';
                else if($data['coin']<2500)
                    $data['degree']='金牌会员';
                else
                    $data['degree']='钻石会员';
            }
            ...
            if ($r!==false) {
                $this->success("充值成功！",U('user/profile/center'));
            } else {
                $this->error("充值失败！");
            }
        }
    		...
    		if(sp_compare_password($old_password, $admin['user_pass'])){
    			if($password==I('post.repassword')){
    				if(sp_compare_password($password, $admin['user_pass'])){
    					...
    					$r=$this->users_model->save($data);
    					if ($r!==false) {
    						$this->success("修改成功！",U('user/profile/center'));
    					} else {
    						$this->error("修改失败！");
    					}
    				}
